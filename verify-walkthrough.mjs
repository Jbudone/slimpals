...
function warn(label) {
	console.log(`⚠️  ${label}`)
	findings.push(`⚠️  ${label}`)
}
function probe(label, detail = "") {
	console.log(`🔍 ${label}${detail ? ` — ${detail}` : ""}`)
}
const ss = async (page, name) => {
	await page.screenshot({ path: join(SS, `${name}.png`) })
}
...
const streakEl = await page
	.locator(".streak-count")
	.isVisible()
	.catch(() => false)
...
if (checkinBtn) {
	...
	const count2 = await page
		...
		.textContent()
		.catch(() => null)
	const doneMsg = await page
		.locator(".done-msg")
		.isVisible()
		.catch(() => false)
	const btnGone = !(await page
		.locator(".checkin-btn")
		.isVisible()
		.catch(() => false))
...
await page.reload()
await page.waitForTimeout(800)
const reloadCount = await page
	.locator(".streak-count")
	.textContent()
	.catch(() => null)
const btnAfterReload = await page
	.locator(".checkin-btn")
	.isVisible()
	.catch(() => false)
...
const rows1 = await page
	.locator(".history-table tbody tr")
	.count()
	.catch(() => 0)
if (rows1 >= 1)
	ok("First weight entry shows in history table", `${rows1} row(s)`)
else bad("No history row after first entry")

// Empty-state chart with 1 entry
const emptyMsg = await page
	.locator("text=Log one more entry to see your chart.")
	.isVisible()
	.catch(() => false)
...
const svgPath = await page
	...
	.catch(() => 0)
if (svgPath > 0)
	ok("D3 chart renders SVG path after 2 entries", `${svgPath} path(s)`)
...
const firstRowText = await page
	...
	.textContent()
	.catch(() => "")
if (firstRowText.includes("81.8"))
	ok("History table is reverse-chronological (81.8 kg first)")
else warn(`First row: "${firstRowText.trim().slice(0, 60)}" — expected 81.8 kg`)

// Auto-checkin idempotence
await page.goto(`${BASE}/`)
await page.waitForTimeout(700)
const streakAfterWeight = await page
	.locator(".streak-count")
	.textContent()
	.catch(() => null)
if (streakAfterWeight === "1")
	ok("Auto-checkin from weight is idempotent (streak stays 1)")
...
if (initDisabled === true) ok("Analyse button disabled with no file")
	...
	bad(
		"Analyse button should be disabled initially",
...
await page
	...
	.setInputFiles("/tmp/test-meal-real.jpg")
await page.waitForTimeout(400)
...
try {
	...
	if (hasResult) {
		const foodName = await page
			.locator(".result-header h3")
			.textContent()
			.catch(() => "")
		const badge = await page
			.locator(".rating-badge")
			.isVisible()
			.catch(() => false)
		...
		const logItems = await page
			.locator(".log-item")
			.count()
			.catch(() => 0)
		if (logItems >= 1)
			ok("Food history entry appears after analysis", `${logItems} item(s)`)
		...
		const isQuota =
			...
			errTxt.toLowerCase().includes("exceeded")
		if (isQuota) {
			ok(
				"Food error path: server returns specific quota message (not opaque error)",
				errTxt.trim().slice(0, 80),
			)
			probe(
				"Gemini free-tier quota exhausted — expected on shared dev key; UI shows correct message",
			)
		} else {
			bad(
				"Food analysis returned unexpected error",
				errTxt.trim().slice(0, 120),
			)
		}
...
await page.goto(`${BASE}/food`)
await page.waitForTimeout(400)
const freshDisabled = await page
	.locator(".analyze-btn")
	.isDisabled()
	.catch(() => null)
if (freshDisabled === true)
	probe("Analyse button correctly disabled on fresh page load")
