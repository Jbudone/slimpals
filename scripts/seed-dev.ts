...
async function seedSocialPosts(userId: string) {
  ...
  await db.insert(schema.socialPosts).values([
    ...
    },
    {
      userId,
      type:      "food_photo",
      content:   { foodName: "Caesar Salad", macros: { calories: 350, protein: 12, carbs: 20, fat: 24 }, mealType: "lunch" },
      createdAt: new Date(now - 1 * 3_600_000),
    },
    {
      userId,
      type:      "ai_message",
      content:   { message: "You're on a roll! Three days in a row — keep it up!" },
