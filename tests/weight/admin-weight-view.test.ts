...
async function registerAndLogin(email: string, name: string) {
	const inviteCode = `ADMIN-WEIGHT-VIEW-INVITE-${++inviteCounter}`
...
async function seedEntry(
	...
	await db
		...
		.values({ userId, weightKg: weightKgTenths, recordedAt, note })
}
...
describe("GET /api/admin/users/:id/weight", () => {
	it("returns 401 without auth", async () => {
		const res = await request(app).get("/api/admin/users/x/weight")
	...
	it("returns an empty list for a user with no entries", async () => {
		...
		const res = await request(app)
			.get(`/api/admin/users/${memberId}/weight`)
			.set("Cookie", adminCookie)
		expect(res.status).toBe(200)
		expect(res.body).toEqual([])
	})

	it("lists entries most-recent-first with decimal kg conversion", async () => {
		const { adminCookie, memberId } = await adminAndMember()
		const older = new Date(Date.now() - 86_400_000)
		const newer = new Date()
		await seedEntry(memberId, 845, older, "Morning")
		await seedEntry(memberId, 840, newer, null)
		...
		expect(res.body[0].weightKg).toBe(84)
		expect(res.body[0].note).toBeNull()
		expect(res.body[1].weightKg).toBe(84.5)
		expect(res.body[1].note).toBe("Morning")
	})
	...
	it("only returns entries for the requested user", async () => {
		...
		await seedEntry(memberId, 800, new Date())
		await seedEntry(otherUserId, 900, new Date())
		...
		expect(res.body[0].weightKg).toBe(80)
	})
})
