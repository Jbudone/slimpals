...
- **No walk-cycle, no 4-directional art** — deliberately deferred per `docs/prd-gym-pixel-art-redesign.md`: full directional sets would multiply NPC art requirements ~4x for a payoff that's easy to defer; revisit only if flip-only visibly reads wrong once real art is in.
- **Planned (not yet built) — exercise pose art:** the PRD specs an *optional, additive* per-(NPC, equipment) pose system — manifest entries keyed `npc_<npcKey>_<equipmentKey>` (e.g. `npc_trainer_marcus_cardio_treadmill`), same category/frame rules as any other sprite, falling back to today's generic bob tween when absent. Don't draw these speculatively — they only get added to the manifest (and this checklist) once the code side (`resolvePoseKey`, pose-vs-fallback resolution, `NpcSprite` becoming a `Sprite`) is built and a specific NPC/equipment pairing is chosen.
