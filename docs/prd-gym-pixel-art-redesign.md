...
- **`GymScene.ts`** equipment animation objects should be built from manifest data (frame count, fps, key) at scene init, not hardcoded per-equipment-key — this is separate from the loading/placeholder work and still open (tracked as its own milestone).
